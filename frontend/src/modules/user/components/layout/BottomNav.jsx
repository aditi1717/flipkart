...
const BottomNav = () => {
    ...
    return (
        <footer className="fixed bottom-0 left-0 right-0 w-full bg-white/95 backdrop-blur-md border-t border-blue-50/50 flex items-center justify-between px-6 py-3 z-50 md:hidden rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.08)]">
            {navItems.map((item) => (
                <NavLink
                    key={item.name}
                    to={item.path}
