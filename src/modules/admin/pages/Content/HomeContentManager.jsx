...
import { MdViewAgenda, MdViewCarousel } from 'react-icons/md';
import { useLocation } from 'react-router-dom';

const HomeContentManager = () => {
    const location = useLocation();
    const [activeTab, setActiveTab] = useState(location.pathname.includes('banners') ? 'banners' : 'sections');

    useEffect(() => {
        setActiveTab(location.pathname.includes('banners') ? 'banners' : 'sections');
    }, [location]);
    ...
    return (
        <div className="space-y-4 max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-3 rounded-2xl border border-gray-100 shadow-sm mb-2">
                <div>
                    <h1 className="text-base font-black text-gray-900 flex items-center gap-2 uppercase tracking-tight">
                        {activeTab === 'sections' ? <MdViewAgenda size={20} className="text-blue-500" /> : <MdViewCarousel size={20} className="text-purple-500" />}
                        Homepage Content
                    </h1>
                </div>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                {activeTab === 'sections' ? <HomeSections /> : <HomeBanners />}
            </div>
        </div>
    );
};
