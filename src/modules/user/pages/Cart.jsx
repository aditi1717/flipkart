    ...
    return (
        <div className="bg-[#f1f3f6] min-h-screen pb-24">
            ...
            {cart.length === 0 && savedForLater.length === 0 ? (
                <div className="flex flex-col items-center justify-center pt-20 px-10 text-center bg-white h-[70vh]">
                ...
                <div className="space-y-2">
                    ...
                    <div className="bg-white px-4 py-3 flex items-center justify-between border-b">
                        <div className="flex flex-col">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] bg-gray-100 px-1.5 py-0.5 rounded text-gray-500 font-bold uppercase">{addresses[0]?.type}</span>
                            </div>
                            <p className="text-[11px] text-gray-500 truncate max-w-[200px]">{addresses[0]?.address}, {addresses[0]?.city}</p>
                        </div>
                        <button onClick={() => navigate('/checkout')} className="text-blue-600 font-bold text-[12px] border px-3 py-1.5 rounded-sm active:bg-blue-50">Change</button>
                    </div>
                    ...
                    <div className="bg-white">
                        {cart.map((item) => (
                            <div key={`${item.id}-${JSON.stringify(item.selectedSize)}`} className="p-4 border-b">
                                <div className="flex gap-4">
                                    <div className="w-20 h-24 flex-shrink-0 bg-gray-50 rounded border p-1">
                                        <img src={item.image} alt={item.name} className="w-full h-full object-contain" />
                                    </div>
                                    <div className="flex-1">
                                        <h2 className="text-sm text-gray-800 line-clamp-2">{item.name}</h2>
                                ...
                                <div className="flex items-center gap-4 mt-6">
                                    ...
                                    <div className="flex items-center gap-6 text-sm font-bold text-gray-800 uppercase">
                                        <button onClick={() => moveToSavedForLater(item)}>Save for later</button>
                                        <button onClick={() => removeFromCart(item.id, { selectedSize: item.selectedSize, selectedColor: item.selectedColor })}>Remove</button>
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    {savedForLater.length > 0 && (
                        <div className="bg-white">
                            ...
                            {savedForLater.map((item) => (
                                <div key={item.id} className="p-4 border-b flex gap-4 opacity-80">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-6 mt-4 text-xs font-bold uppercase">
                                            <button onClick={() => moveToCart(item)} className="text-blue-600">Move to Cart</button>
                                            <button onClick={() => removeFromSavedForLater(item.id)} className="text-gray-500">Remove</button>
                                        </div>
                                    </div>
                                </div>
                            ))}
