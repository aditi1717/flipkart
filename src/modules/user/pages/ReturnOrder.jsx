...
const ReturnOrder = () => {
    ...
    return (
        <div className="bg-[#f1f3f6] min-h-screen">
            <div className="bg-white px-4 py-4 flex items-center gap-2 border-b sticky top-0 z-50 shadow-sm">
                <button
                    onClick={() => navigate(-1)}
            ...
            <div className="bg-white m-2 rounded-xl overflow-hidden shadow-sm border border-gray-100">
                ...
                <div className="divide-y divide-gray-50">
                    {targetItems.map((item, idx) => (
                        <div key={idx} className="flex gap-4 p-4">
                            <div className="w-16 h-16 flex-shrink-0 bg-white rounded-lg border border-gray-100 p-1">
                                <img src={item.image} alt="" className="w-full h-full object-contain" />
                            </div>
                            <div className="flex-1">
                                <h2 className="text-[13px] font-bold text-gray-800 line-clamp-2 leading-tight">{item.name}</h2>
                                <p className="text-[10px] text-gray-400 mt-1 font-bold uppercase">₹{item.price.toLocaleString()} • Qty: {item.quantity}</p>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {step === 1 && (
                <div className="bg-white m-2 rounded-xl shadow-sm border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="divide-y divide-gray-50">
                        {reasons.map((r) => (
                            <label key={r.title} className="flex items-center justify-between px-4 py-4 cursor-pointer active:bg-gray-50 transition-colors">
            ...
            {step === 2 && (
                <div className="bg-white m-2 rounded-xl shadow-sm border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-right-4 duration-500">
                    <div className="px-4 py-4 bg-blue-600 text-white flex items-center justify-between">
                        ...
                        <button onClick={() => setStep(1)} className="text-[10px] font-black uppercase text-white/80 border border-white/30 px-2 py-1 rounded">Change</button>
                    </div>
                    <div className="p-4">
                        ...
                        <div className="space-y-2">
                            {reasons.find(r => r.title === reason)?.subs.map(sub => (
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-3 rounded-xl border text-sm transition-all ${subReason === sub ? 'border-blue-600 bg-blue-50 text-blue-700 font-bold' : 'border-gray-100 text-gray-600'}`}
                                >
                                    {sub}
                                </button>
                            ))}
            ...
            {step === 3 && (
                <div className="m-2 space-y-2 pb-10 animate-in fade-in slide-in-from-right-4 duration-500">
                    {/* Resolution Section */}
                        ...
                        <div className="p-4">
                            <div className="grid grid-cols-1 gap-4">
                                ...
                                <button
                                    ...
                                    className={`group relative p-4 rounded-xl border-2 text-left transition-all ${returnMode === 'REFUND' ? 'border-blue-600 bg-blue-50' : 'border-gray-100'}`}
                                >
                                    <div className="flex items-start gap-4">
                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${returnMode === 'REFUND' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-400'}`}>
                                        ...
                                        {returnMode === 'REFUND' && <span className="material-icons text-blue-600">check_circle</span>}
                                    </div>
                                </button>
                                ...
                                <button
                                    ...
                                    className={`group relative p-4 rounded-xl border-2 text-left transition-all ${returnMode === 'REPLACEMENT' ? 'border-blue-600 bg-blue-50' : 'border-gray-100'}`}
                                >
                                    <div className="flex items-start gap-4">
                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${returnMode === 'REPLACEMENT' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-400'}`}>
                                                        ...
                                                        <div className="flex flex-wrap gap-2">
                                                            {['S', 'M', 'L', 'XL', 'XXL'].map(s => (
                                                                <button
                                                                    ...
                                                                    className={`min-w-[40px] h-10 border rounded-lg text-xs font-bold transition-all ${selectedReplacementSize === s ? 'bg-blue-600 border-blue-600 text-white shadow-md' : 'bg-white border-gray-200 text-gray-600'}`}
                                                                >
                                                                    {s}
                                                                </button>
                                                            ))}
                                                        ...
                                                        <div className="flex flex-wrap gap-3">
                                                            {[
                                                                ...
                                                                <button
                                                                    ...
                                                                    className={`flex items-center gap-2 px-3 py-2 border rounded-full text-[10px] font-bold uppercase tracking-wider transition-all ${selectedReplacementColor === c.name ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-gray-100 bg-white text-gray-500'}`}
                                                                >
                                        ...
                                        {returnMode === 'REPLACEMENT' && <span className="material-icons text-blue-600">check_circle</span>}
                                    </div>
                                </button>
                            </div>
                            ...
                            <button
                                ...
                                className="w-full bg-[#fb641b] text-white py-4 rounded-xl font-black uppercase mt-8 shadow-xl shadow-[#fb641b]/20 active:scale-95 transition-all text-sm tracking-widest"
                            >
                    ...
                    <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <span className="text-blue-600 text-[10px] font-black uppercase">Change</span>
                        </div>
